...
Handles data retrieval for the Seller, Homepage, and Admin dashboards.
...
class SellerDashboard(MethodView):
    """
    Class-based view for retrieving Seller Dashboard metrics and inventory info.
    """
    @jwt_required
...
class HomepageDashboard(MethodView):
    """
    Class-based view for retrieving products for the storefront homepage.
    """
    @jwt_required
    @limiter.limit("3 per second")
    def get(self):
        ...
        products_data = dashboard_service.get_homepage_products(
...
class AdminDashboard(MethodView):
    """
    Class-based view for retrieving metrics and trends for the Admin Console.
    """
    @jwt_required
    @limiter.limit("3 per second")
    def get(self):
        ...
        requesting_user = (
            g.db.query(UserModel).filter(UserModel.id == g.user_id).first()
        )
        if not requesting_user or not requesting_user.is_admin:
            return make_response(
                jsonify({"success": False, "message": "Admin privileges required"}), 403
            )
        ...
        total_orders = (
            g.db.query(OrderModel).filter(OrderModel.status.in_(paid_statuses)).count()
        )

        total_revenue_val = (
            g.db.query(func.sum(OrderModel.total_amount))
            .filter(OrderModel.status.in_(paid_statuses))
            .scalar()
            or 0.0
        )
        ...
        # Get all paid orders
        orders = (
            g.db.query(OrderModel.created_at, OrderModel.total_amount)
            .filter(OrderModel.status.in_(paid_statuses))
            .all()
        )
        ...
        weekly_data = [
            {"label": day_names[d], "value": amt} for d, amt in weekly_map.items()
        ]
        ...
        monthly_data = [
            {"label": month_names[m - 1], "value": amt}
        ...
        quarterly_data = [
            {"label": f"Q{q}", "value": amt} for q, amt in quarterly_map.items()
        ]
        ...
        yearly_data = [
            {"label": str(yr), "value": amt} for yr, amt in sorted(yearly_map.items())
        ]
