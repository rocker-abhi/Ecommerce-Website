...
Handles placing new orders, managing transactions/payments, and retrieving user order history.
...
# Define Order blueprint
order_bp = Blueprint("order", __name__, url_prefix="/order")


class OrderAPI(MethodView):
    """
    Class-based view for order management.
    Provides GET (retrieve order history) and POST (create/checkout new order) endpoints.
    """
    ...
    def post(self):
        ...
        if not cart or not cart.items:
            return make_response(
                jsonify({"success": False, "message": "Cart is empty"}), 400
            )
        ...
        for item in cart.items:
            ...
            if not product:
                return make_response(
                    jsonify(
                        {"success": False, "message": f"Product {product_id} not found"}
                    ),
                    400,
                )
            ...
            order_item = OrderItemModel(
                product_id=product.id, quantity=quantity, price=item_price
            )
        ...
        # 3. Create the Order
        order = OrderModel(
            user_id=user_id, total_amount=total_amount, status=OrderStatus.PAID
        )
